...
var door_der : bool = false # Детектор для нахождение в области двери
...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("interact_f") and door_der:
...
func _on_door_in_derevna_body_entered(body: Node2D) -> void:
	door_der = true
	object_f.visible = true


func _on_door_in_derevna_body_exited(body: Node2D) -> void:
	door_der = false
