...
func _ready() -> void:
	...
	tween_music_start.tween_property(music, "volume_db", -10.0, 4)
...
func _process(delta: float) -> void:
	if Input.is_action_just_pressed("interact_f") and door_ch:
		...
		music.stop()
		get_tree().change_scene_to_file("res://Levels/Level_2_1.tscn")	


func _on_door_in_level_2_1_body_entered(body: Node2D) -> void:
	door_ch = true
	object_f.visible = true


func _on_door_in_level_2_1_body_exited(body: Node2D) -> void:
	door_ch = false
